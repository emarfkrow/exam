SELECT
    ...
    , a.`SAIKI_MEI`
    , a.`IDREF_ID`
    , (SELECT r0.`IDREF_MEI` FROM TB4_ID r0 WHERE r0.`IDREF_ID` = a.`IDREF_ID`) AS `IDREF_MEI`
    , TRIM(TRAILING ' ' FROM a.`CDREF_CD`) AS `CDREF_CD`
    , (SELECT r1.`CDREF_MEI` FROM TB4_CD r1 WHERE r1.`CDREF_CD` = a.`CDREF_CD`) AS `CDREF_MEI`
