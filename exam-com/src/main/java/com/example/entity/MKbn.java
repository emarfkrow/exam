 ...
 * @author emarfkrow
 */
public class MKbn implements IEntity {
     ...
     * @return 区分マスタ
     */
    public static MKbn get(final Object param1) {
        ...
        return Queries.get(sql, map, MKbn.class);
    }
    ...
    public int insert(final LocalDateTime now, final String execId) {
        ...
        // 区分値マスタの登録
        if (this.mKbnValues != null) {
            for (MKbnValue mKbnValue : this.mKbnValues) {
                mKbnValue.setKbnNm(this.getKbnNm());
                mKbnValue.insert(now, execId);
            }
        }

        // 区分マスタの登録
        String sql = "INSERT INTO m_kbn(\r\n      " + names() + "\r\n) VALUES (\r\n      " + values() + "\r\n)";
    ...
    public int update(final LocalDateTime now, final String execId) {
        ...
        if (this.mKbnValues != null) {
            for (MKbnValue mKbnValue : this.mKbnValues) {
                ...
                mKbnValue.setKbnNm(this.kbnNm);
                try {
                    mKbnValue.insert(now, execId);
                } catch (Exception e) {
                    mKbnValue.update(now, execId);
                }
            }
        }

        // 区分マスタの登録
        String sql = "UPDATE m_kbn\r\nSET\r\n      " + getSet() + "\r\nWHERE\r\n    " + getWhere();
        return Queries.regist(sql, toMap(now, execId));
    }
    ...
    public int delete() {
        ...
        // 区分マスタの削除
        String sql = "DELETE FROM m_kbn WHERE " + getWhere();
    ...
    /** 区分値マスタのリスト */
    private List<MKbnValue> mKbnValues;

    /** @return 区分値マスタのリスト */
    @com.fasterxml.jackson.annotation.JsonProperty("MKbnValues")
    public List<MKbnValue> getMKbnValues() {
        return this.mKbnValues;
    }

    /** @param list 区分値マスタのリスト */
    public void setMKbnValues(final List<MKbnValue> list) {
        this.mKbnValues = list;
    }

    /** @param mKbnValue */
    public void addMKbnValues(final MKbnValue mKbnValue) {
        if (this.mKbnValues == null) {
            this.mKbnValues = new ArrayList<MKbnValue>();
        }
        this.mKbnValues.add(mKbnValue);
    }

    /** @return 区分値マスタのリスト */
    public List<MKbnValue> referMKbnValues() {
        if (this.mKbnValues == null) {
            this.mKbnValues = MKbn.referMKbnValues(this.kbnNm);
        }
        return this.mKbnValues;
    }
    ...
    /**
     * @param param1 kbnNm
     * @return List<MKbnValue>
     */
    public static List<MKbnValue> referMKbnValues(final String param1) {
        List<String> whereList = new ArrayList<String>();
        whereList.add("KBN_NM = :kbn_nm");
        String sql = "SELECT * FROM m_kbn_value WHERE " + String.join(" AND ", whereList);
        Map<String, Object> map = new HashMap<String, Object>();
        map.put("kbn_nm", param1);
        return Queries.select(sql, map, MKbnValue.class, null, null);
    }
}
