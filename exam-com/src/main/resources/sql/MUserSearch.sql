...
WHERE
    ...
    AND TRIM (a.`USER_SEI`) LIKE CONCAT ('%', :user_sei, '%') 
    AND TRIM (a.`USER_MEI`) LIKE CONCAT ('%', :user_mei, '%') 
    AND TRIM (a.`EMAIL`) LIKE CONCAT ('%', :email, '%') 
    AND TRIM (a.`PASSWORD`) LIKE CONCAT ('%', :password, '%') 
