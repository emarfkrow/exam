...
WHERE
    ...
    AND TRIM(TRAILING ' ' FROM a.`NULL_ENTITY2_MEI`) LIKE CONCAT ('%', :null_entity_2_mei, '%') 
    ...
    AND TRIM(TRAILING ' ' FROM a.`HIDUKE_YMD`) LIKE CONCAT ('%', :hiduke_ymd, '%') 
    ...
    AND TRIM(TRAILING ' ' FROM a.`SAMPLE_Y`) LIKE CONCAT ('%', :sample_y, '%') 
    AND a.`SAMPLE_Y` >= :sample_y_1 
    AND a.`SAMPLE_Y` <= :sample_y_2 
    AND TRIM(TRAILING ' ' FROM a.`SAMPLE_M`) LIKE CONCAT ('%', :sample_m, '%') 
    ...
    AND TRIM(TRAILING ' ' FROM a.`JIKOKU_HM`) LIKE CONCAT ('%', :jikoku_hm, '%') 
    ...
    AND TRIM(TRAILING ' ' FROM a.`FIG_8_BI`) LIKE CONCAT ('%', :fig_8_bi, '%') 
